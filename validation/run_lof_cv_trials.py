#!/usr/bin/env python3
"""
validation/run_lof_cv_trials.py

K-Fold Cross-Validation for Local Outlier Factor (LOF) novelty detection (unsupervised),
...
- For each seed:
    ...
    - Train LOF ONLY on normal samples (y==0) in training fold
    - Evaluate on validation fold
- Writes per-fold detailed CSV: validation/lof_runs/lof_runs_detailed.csv
"""
...
def main():
    ...
    ap.add_argument("--contam", type=float, default=0.05, help="LOF contamination")
    ap.add_argument("--neighbors", type=int, default=20, help="LOF n_neighbors")
    ap.add_argument("--max-rows", type=int, default=None, help="Optional: limit rows for faster runs")
    ap.add_argument("--outdir", default="validation/lof_runs", help="Output folder")
    args = ap.parse_args()
    ...
    for seed in seeds:
        ...
        for fold_i, (train_idx, val_idx) in enumerate(skf.split(X, y), start=1):
            ...
            # Guard: LOF requires n_neighbors < n_samples
            # (and n_neighbors >= 2)
            n_neighbors = int(args.neighbors)
            n_neighbors = max(2, min(n_neighbors, max(2, n_train_normal - 1)))
            ...
            y_hat = (pred == -1).astype(int)

            # decision_function: larger => more normal
            # convert to anomaly score where larger => more anomalous
            ...
            rows.append({
                ...
                "fold": fold_i,
                "folds": folds,
                "contamination": float(args.contam),
    ...
    detailed_path = outdir / "lof_runs_detailed.csv"
